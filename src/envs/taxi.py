...
class TaxiEnv(Env):
    ...
    * v0: Initial versions release
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        self.ACTIONS = {'SOUTH': 0, 'NORTH': 1, 'EAST': 2, 'WEST': 3, 'PICKUP': 4, 'DROPOFF': 5}
        self.OBJECTS = {'R':0, 'G':1, 'Y':2, 'B':3, 'T ':4, ' ':5, 'TR':6, 'TG':7, 'TY':8, 'TB':9}
    ...
    def render_show(self, state):
        ...
        out[1 + di][2 * dj + 1] = utils.colorize(out[1 + di][2 * dj + 1], "magenta")
        outfile.write("\n".join(["".join(row) for row in out]) + "\n")
        # if state.lastaction is not None:
        #     outfile.write(
        #         f"  ({['South', 'North', 'East', 'West', 'Pickup', 'Dropoff'][self.lastaction]})\n"
        #     )
        # else:
    ...
    def check_done(self, state):
        ...
        action = self.select_optimal_action(q_table, state[0])
        if action == 5:
            print("DONE") 
            return True
        else:
            return False
    def set_state(self, statearry):
        self.reset()
        self.s = statearry[0]
        return
         
    def equal_states(self, statearry, state):
        if(statearry[0] == state):
            return True
        else: 
            return False
    ...
    def actionable(self, x, fact):
        factdecoded = list(self.decode(fact[0]))
        prposeddecoded = list(self.decode(x[0]))

        #print('Fact:', factdecoded)
        #print('Proposed:', prposeddecoded)

        #Proposed is possible move
        if abs(factdecoded[0]-prposeddecoded[0]) + abs(factdecoded[1]-prposeddecoded[1]) != 1:
           # print("FALSE")
            return False

        #Proposed is not the same move
        if factdecoded[0] == prposeddecoded[0] and factdecoded[1] == prposeddecoded[1]:
        #     #print("FALSE")
             return False
        
        
        #print(self.render_show(self.state))
        #print(self.render_show(x[0]))

        #print('Current:', currentdecoded)
        #print('Proposed:', prposeddecoded)
